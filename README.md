...
- [ ] Set the bundle to be output as filename `webpack-bundle.js` in the `build/` folder. The Webpack build process, after it runs, will make a file in the `build` folder called `webpack-bundle.js`.
...
- [ ] Change the mode in your `webpack.config.js` to development (if you haven't already). Also delete the `webpack-bundle.js` file in the build folder (so that, for debugging purposes, you know for sure that any content you serve is from `webpack-dev-server`).
- [ ] Run `npm run dev`. The web page automatically opens because we're running `webpack-dev-server --open`. We should get a 404 error for the route `/build/webpack-bundle.js`.
- [ ] How can we get rid of this error? Hint: look up the `devServer` setting for `webpack.config.js` and see if we can modify the route on which Webpack output is served.
...
1. In our npm script for `dev`, we need to run the express server (preferably using nodemon rather than node) at the same time that webpack-dev-server runs. Windows users may have to do this using the [concurrently](https://www.npmjs.com/package/concurrently) module.
